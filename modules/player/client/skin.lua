...
local Components = {
	...
	{label = "Nose Peak Height",			name = 'nose_2',			value = 0.0,	min = 0,	zoomOffset = 0.6,		camOffset = 0.65},
...
function LoadDefaultModel(malePed, cb)
	...
	if characterModel then
		while not HasModelLoaded(characterModel) do RequestModel(characterModel) Wait(0) end
...
function ApplySkin(skin, clothes)
	...
	SetPedHeadBlendData(playerPed, Character['dad'], Character['mom'], nil, Character['mom'], Character['dad'], nil, Character['face'], Character['skin'], nil, true);
...
AddEventHandler('skinchanger:change', function(key, val)
	...
	if key == 'sex' then
		...
		-- print("APPLY NEW SKIN", key, val, "CHARACTER TABLE", Character[key]);
		ApplySkin(Character)
	end
end)
