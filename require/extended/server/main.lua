...
Citizen.CreateThread(function()
	...
	local query = '`accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `id_unique`' -- Select these fields from the database
	if extendedConfig.Multichar or extendedConfig.Identity then	-- append these fields to the select query
		query = query..', `firstname`, `lastname`, `dateofbirth`, `sex`, `height`'
	end

	if extendedConfig.Multichar then -- insert identity data with creation
...
if extendedConfig.Multichar then
...
function createESXPlayer(identifier, playerId, data)
	...
	for account,money in pairs(extendedConfig.StartingAccountMoney) do
	...
	if not extendedConfig.Multichar then
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll(LoadPlayer, { identifier
			...
			for account,label in pairs(extendedConfig.Accounts) do
				table.insert(userData.accounts, {
					name = account,
					money = foundAccounts[account] or extendedConfig.StartingAccountMoney[account] or 0,
					label = label
				})
			end
			...
			-- Id Unique
			if result[1].id_unique ~= nil then
				userData.id_unique = result[1].id_unique
			end
	...
	Async.parallel(tasks, function(results)
		local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.loadout, userData.playerName, userData.coords, userData.id_unique)
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sourceXPlayer.showNotification(_U('gave_account_money', ESX.Math.GroupDigits(itemCount), extendedConfig.Accounts[itemName], targetXPlayer.name))
			targetXPlayer.showNotification(_U('received_account_money', ESX.Math.GroupDigits(itemCount), extendedConfig.Accounts[itemName], sourceXPlayer.name))
		else
