...
Citizen.CreateThread(function()
	while not extendedConfig.Multichar do
		Citizen.Wait(0)
...
AddEventHandler('esx:playerLoaded', function(xPlayer, isNew, skin)
	...
	-- enable PVP
	if extendedConfig.EnablePVP then
		SetCanAttackFriendly(ESX.PlayerData.ped, true, false)
		NetworkSetFriendlyFireOption(true)
	end

	if extendedConfig.EnableHud then
		...
		for i = 1, 3 do 
			local zeub = ESX.PlayerData.accounts;
			if zeub[i].name == "bank" then
				zeub[i]["className"] = "fas fa-wallet";
				zeub[i].color = "blue";
			elseif zeub[i].name == "money" then
				zeub[i]["className"] = "fas fa-money-bill-wave-alt";
				zeub[i].color = "green";
			elseif zeub[i].name == "black_money" then
				zeub[i]["className"] = "fas fa-money-bill-alt";
				zeub[i].color = "red";
			end
		end
		
		-- for k,v in ipairs(ESX.PlayerData.accounts) do
		-- 	local accountTpl = '<div><i class="' ..v.className.. '" style="color:' ..v.color.. ';"/>&nbsp;{{money}}</div>'
...
AddEventHandler('esx:onPlayerLogout', function()
	...
	if extendedConfig.EnableHud then ESX.UI.HUD.Reset() end
end)
...
if extendedConfig.EnableDefaultInventory then
...
if not extendedConfig.EnableWantedLevel then
